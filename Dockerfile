...
RUN ANTHROPIC_CLEAN="$(printf '%s' "${ANTHROPIC_API_KEY}" | tr -d '\n\r')" && \
    ...
    ELEVENLABS_CLEAN="$(printf '%s' "${ELEVENLABS_API_KEY}" | tr -d '\n\r')" && \
    ELEVENLABS_VOICE_CLEAN="$(printf '%s' "${ELEVENLABS_VOICE_ID}" | tr -d '\n\r')" && \
    ...
    printf '{\n  "ANTHROPIC_API_KEY": "%s",\n  "OPENAI_API_KEY": "%s",\n  "ELEVENLABS_API_KEY": "%s",\n  "ELEVENLABS_VOICE_ID": "%s",\n  "SPOONACULAR_API_KEY": "%s",\n  "GOOGLE_CSE_API_KEY": "%s",\n  "PEXELS_API_KEY": "%s",\n  "NEON_PASSWORD": "%s"\n}\n' \
      "${ANTHROPIC_CLEAN}" "${OPENAI_CLEAN}" "${ELEVENLABS_CLEAN}" "${ELEVENLABS_VOICE_CLEAN}" "${SPOONACULAR_CLEAN}" "${GOOGLE_CSE_CLEAN}" "${PEXELS_CLEAN}" "${NEON_CLEAN}" \
