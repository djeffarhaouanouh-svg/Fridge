...
class NeonService {
  ...
  /// Charge des recettes depuis la table Neon `recipes_marmiton`.
  /// Le mapping est volontairement tolérant aux noms de colonnes.
  Future<List<Meal>> loadMarmitonRecipes({int limit = 3}) async {
    final rows = await query(
      '''
    ...
    for (var i = 0; i < rows.length; i++) {
      ...
      List<String> pickSteps() {
        final dynamic rawSteps = map['steps_json'] ?? map['steps'] ?? map['instructions'];
        if (rawSteps is List) {
          return rawSteps.map((e) => e.toString()).where((e) => e.isNotEmpty).toList();
        }
      ...
      final duration = pickInt(['duration', 'duration_min', 'time', 'time_min', 'ready_in_minutes'], fallback: 20);
      ...
      final typeLabel = pickString(['type_label'], fallback: type == 'balanced' ? 'Équilibré' : 'Simple');
      final emoji = pickString(['emoji'], fallback: i == 0 ? '🍝' : '🍽️');
      final color = pickString(['color'], fallback: '#82D28C');
      final photo = pickString(['image_url', 'image', 'photo', 'thumbnail'], fallback: '');
